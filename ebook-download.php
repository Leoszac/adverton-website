...
function denyDownload(string $reason): void {
    error_log("[adverton-ebook-download] $reason");
    ...
    echo '<!doctype html><html><head><meta charset="utf-8"><title>Link expired — Adverton</title>'
       ...
       . '<p>Download links for the Growth Engine guide expire monthly to keep the file private. '
       . 'Request a fresh copy at <a href="/growth-engine.html">adverton.net/growth-engine</a> — '
...
// Verify HMAC: signed against email|ebook|YYYY-MM
// Try current month first, then previous (so links from end of month don't break the next day).
$salt = config('UNSUBSCRIBE_SALT') ?: 'adverton-default-salt';
$emailNorm = strtolower(trim($email));
$valid = false;
foreach ([0, -1] as $monthOffset) {
    $ts = strtotime(($monthOffset === 0 ? 'now' : "$monthOffset month"));
    $month = gmdate('Y-m', $ts);
    $expected = substr(hash_hmac('sha256', $emailNorm . '|ebook|' . $month, $salt), 0, 32);
    if (hash_equals($expected, $sig)) { $valid = true; break; }
}
if (!$valid) denyDownload("signature mismatch for $email");

if (!is_readable(PDF_PATH)) {
    error_log('[adverton-ebook-download] PDF not found at ' . PDF_PATH);
